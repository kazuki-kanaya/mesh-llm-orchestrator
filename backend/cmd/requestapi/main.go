...
import (
	...
	"github.com/kazuki-kanaya/mesh-llm-orchestrator/backend/internal/requestapi/app"
)
...
func main() {
	...
	if err := app.Run(ctx, app.Config{
		...
		HTTPAddr:         mustGetEnv("REQUESTAPI_ADDR"),
		TargetBaseURL:    mustGetEnv("TARGET_BASE_URL"),
		WaitTimeout:      mustDurationEnv("REQUESTAPI_WAIT_TIMEOUT"),
	}); err != nil {
		log.Fatal(err)
	}
}
